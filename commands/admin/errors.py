...
async def cmderror(cmd, error, owners):
    retry_after = 1
...
class errhandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        adms = [613089075250987168, 648627663115911193]
        ...
        try:
            ...
            text = await cmderror(ctx, error, adms) #app.owner)
            emb = Embed(colour=0xc31b21, description='❌ '+text)
            emb.set_author(name=ctx.author.name+'#'+ctx.author.discriminator, icon_url=ctx.author.avatar)
